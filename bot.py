...
while True:
    try:
        ...
        if can_buy == False and can_sell == False:
            ...
            if spike_fluct <= 0 and abs(spike_fluct) >= 30 and position == 'neutral':
                ...
                save_price_history(price_history)
            elif spike_fluct >= 0 and spike_fluct >= 30 and position == 'neutral':
        ...
        if can_sell == False and can_buy == True and position == "down":
            ...
            #tuka prodavame
            if token_price < low_point:
                low_point = token_price

            if line_smart_buy == False and token_price < TOKEN_PROFIT_PRICE:
                line_smart_buy = True

            if line_smart_buy == True and token_price > TOKEN_PROFIT_PRICE + 5:
                make_buy_order = True
            
            if token_price > TOKEN_START_PRICE:
                make_buy_order = True
            
            if low_point < TOKEN_PROFIT_PRICE and token_price > low_point + 20:
                make_buy_order = True
            ...
            #tuka prodavame
            if token_price > high_point:
                high_point = token_price

            if line_smart_sell == False and token_price > TOKEN_PROFIT_PRICE:
                line_smart_sell = True

            if line_smart_sell == True and token_price < TOKEN_PROFIT_PRICE - 5:
                make_sell_order = True
            
            if token_price < TOKEN_START_PRICE:
                make_sell_order = True
            
            if high_point > TOKEN_PROFIT_PRICE and token_price < high_point - 20:
                make_sell_order = True
