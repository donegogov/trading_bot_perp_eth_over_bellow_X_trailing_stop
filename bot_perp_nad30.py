...
MARGIN_PER_TRADE = 10  # $30 по позиција
LEVERAGE = 10
...
def detect_spikes_dynamic2(prices, min_spike=3, max_spike=300, recovery_threshold=0.053, max_time_range=240):
    """
...
def detect_spikes_dynamic(prices, min_spike=20, max_spike=300, max_time_range=240):
    """
...
while True:
    try:
        ...
        if can_buy == False and can_sell == False:
            ...
            if spike_fluct <= 0 and abs(spike_fluct) >= 40 and position == 'neutral':
                print(f"🔍 DEBUG: vo if spike_fluct <= 0 and abs(spike_fluct) >= 30")
                threshold = 0.01
                TOKEN_PROFIT_PRICE = token_price - 20
                TOKEN_START_PRICE = token_price + 10  # 3% lower
                ...
                if float(str_token_wallet_balance) > 0.005:
                    str_token_wallet_balance = "0.005"
                side = 'Sell'
                #make_order(SYMBOL, side, str_token_wallet_balance)
                # СЕГА:
                qty = calculate_qty(token_price)
                make_order(SYMBOL, 'Sell', qty, 
                        take_profit=round(TOKEN_PROFIT_PRICE, 2),
                        stop_loss=round(TOKEN_START_PRICE, 2))
                # Trailing: ако цената падне $20 под тековна, активирај trailing
                # Trailing distance = $20 (затвори ако цeната скокне $20 од најниска точка)
                # Stop loss = token_price + 10 (максимална загуба)
                # Activation price = token_price - 20 (trailing почнува кога е во профит $20)
                sleep(2)  # чекај да се отвори позицијата
                set_trailing_stop(
                    symbol=SYMBOL,
                    stop_loss=token_price + 8,       # SL = $10 загуба
                    trailing_distance=10,              # затвори ако скокне $20 од најниска точка
                    activation_price=token_price - 20  # активирај кога е $20 во профит
                )
                #short = len(token_price_history) - short_array
                price_history[COIN] = {"prices": historical_prices[-3:]}
                save_price_history(price_history)
            elif spike_fluct >= 0 and spike_fluct >= 40 and position == 'neutral':
                ...
                if position == "neutral":
                    position = "up"
                if high_point == 0:
                    high_point = token_price
                side = 'Buy'
                #make_order(SYMBOL, side, "10")
                # СЕГА:
                qty = calculate_qty(token_price)
                ...
                # Trailing: 
                # Stop loss = token_price - 10 (максимална загуба)
                # Trailing distance = $20 (затвори ако цената падне $20 од врв)
                # Activation price = token_price + 20 (trailing почнува кога е $20 во профит)
                sleep(2)
                set_trailing_stop(
                    symbol=SYMBOL,
                    stop_loss=token_price - 8,       # SL = $10 загуба
                    trailing_distance=10,              # затвори ако падне $20 од највисока точка
                    activation_price=token_price + 20  # активирај кога е $20 во профит
)
                price_history[COIN] = {"prices": historical_prices[-3:]}
                save_price_history(price_history)
